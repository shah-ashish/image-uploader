...
export default function ImageUploader() {
  ...
  const handleFile = useCallback((f) => {
    ...
    if (!validateImage(f)) {
      setStatus({
        type: "error",
        message: "Unsupported file type. Use an image.",
      });
      return;
    }
  ...
  const onPaste = async (e) => {
    ...
    if (/^https?:\/\/.+\.(png|jpe?g|gif|webp|bmp)/i.test(text)) {
      try {
        ...
        setStatus({
          type: "error",
          message: "Failed to fetch pasted image URL.",
        });
        return;
      }
    }
        ...
        {status.type === "success" && (
          <div className="rounded-md bg-green-50 border border-green-200 p-3 text-green-800">
            ...
            <div className="flex  sm:flex-row sm:items-center sm:justify-between gap-2 bg-white border rounded-md p-2">
              ...
              <div
                className={`w-6 h-6 rounded cursor-pointer flex justify-center items-center text-white ${bg}`}
                onClick={() => {
                  navigator.clipboard.writeText(url);
                  setBg("bg-blue-600"); // change color on click
                }}
              >
                <span
